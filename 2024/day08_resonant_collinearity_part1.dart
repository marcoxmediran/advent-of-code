...
void main() {
  File('input08_small.txt').readAsString().then(
    (String content) {
        ...
        for (int i = 0; i < value.length - 1; i++) {
          ...
          for (int j = i + 1; j < value.length; j++) {
            ...
            var first_coordinate =
                (coordinate.$1 + difference.$1, coordinate.$2 + difference.$2);
            var second_coordinate = (
              next_coordinate.$1 - difference.$1,
              next_coordinate.$2 - difference.$2
            );

            if (IsValidCoordinates(first_coordinate, dimensions) &&
                !antinodes.contains(first_coordinate)) {
              antinodes.add(first_coordinate);
            }
            if (IsValidCoordinates(second_coordinate, dimensions) &&
                !antinodes.contains(second_coordinate)) {
      ...
      print('first_answer: ${antinodes.length}');
    },
  );
}
